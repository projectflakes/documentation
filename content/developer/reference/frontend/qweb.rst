...
``t-cache`` and ``t-nocache``

Why and when to use ``t-cache``?
''''''''''''''''''''''''''''''''

This directive is used to perform performance optimizations. However, this
should be used sparingly. Indeed, ``t-cache`` will inevitably complicate
templates (and their understanding of ``t-set`` for example). The use of
``t-cache`` will avoid making queries to the database by keeping the parts of
the final rendering. For a better efficiency, it will be necessary to think of
putting different values ​​in lazy in the controllers (in order not to take time
for the requests to the database, compute... whereas these elements would not
be used by the rendering because they could be used in a cached template part).
You have to be careful, because you have to be able to invalidate the cache at
the right time and absolutely take into account the fact that different users
can use the same template and therefore the same cached values. **key
expression** should be chosen wisely. Also, using ``t-cache`` is not free.
Indeed, either it will be necessary to carry out additional invalidations, or
in the case of the recordset queried the database to take into account the
``write_date``.
The values ​​in ``t-cache`` are scoped, this implies that if there are ``t-set`` in
this part of the template, the rendering could be different than if there was
no attribute ``t-cache``.
...
``t-cache="record"`` and add to the scoped root values each time.
